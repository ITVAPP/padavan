  ...
  <script>
      ...
      function loadTrafficStats() {
          try {
              ...
              try {
                  var rawData = <% nvram_dump("traffic_stats.json",""); %>;
                  trafficData = rawData || null;
              } catch(e) {
              ...
              try {
                  ...
                  throw new Error('流量统计数据格式不正确，请检查JSON格式');
              }

              // 4. 数据结构验证
              if (!data?.time) {
                  throw new Error('数据缺少时间信息');
              }
              if (!Array.isArray(data?.devices)) {
                  throw new Error('设备数据格式不正确');
              }
              if (!data?.total?.up_formatted || !data?.total?.down_formatted) {
                  throw new Error('总流量数据不完整');
              }
